...
class DRAMConfigFile:
  ...
  def init_values(self):
    ...
    self.data['paths']['path_list_to_group_cfg'] =      []
    self.data['paths']['dramsys']         =             None
    self.data['paths']['zuse_ki_avf']     =             None
  ...
  def read_path_file(self, filename):
    ...
    self.data['paths']['zuse_ki_avf']     = json_obj["paths"].get("zuse_ki_avf",  None)
  ...
  def update_config(self, file_path):
    self.data = self.read_config(file_path)
  ...
  def save(self):
    ...
    for value in values:
      ...
      newdata['paths']['path_list_to_group_cfg'] = [cfg_file]
      file_list.append(cfg_file)

      self.create_experiment(cfg_file, newdata)
    self.data['paths']['path_list_to_group_cfg'] = file_list
    self.create_experiment(main_file, self.data)
...
# TODO: ISS
  ...
  def get_cnn_data(self):
    return {
      'path_to_cnn_conv':       join(self.data['paths']['zuse_ki_avf'], 'vpro_sys_behavioral', 'APPS', 'EISV', 'cnn_converter'),
  ...
  def group(self, file):
    self.update_config(file)
    files = self.data['paths']['path_list_to_group_cfg']
    ...
    newdata = self.data.copy()
    for f in files:
      newdata = self.read_config(f)

      path    = newdata['paths']['path_to_output_parsed']
      for variable in self.variables:
        df_tmp[variable].append(newdata['configs'][variable])
